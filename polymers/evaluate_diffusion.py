...
print(sum(p.numel() for p in model_blip.parameters())/(1024*1024))
...
print(sum(p.numel() for p in bart_model.parameters())/(1024*1024))
print(sum(p.numel() for p in bart_model.parameters())/(1024))
...
trainer.evaluate_test_multi(test_loader, part=False, w =0.8, n_samples=5)
