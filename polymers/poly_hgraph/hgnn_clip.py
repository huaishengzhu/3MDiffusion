...
class HierVAE(nn.Module):
    ...
    def reconstruct(self, batch):
        ...
        graph_feats_all = self.graph_proj(batch_node)
        graph_feats_all = self.encoder.pool(graph_feats_all[batch_mask], graph.batch)
        # graph_feats = graph_feats_all.mean(dim=-2, keepdim=graph_feats_all.dim() <= 2)
        root_vecs = self.new_graph_proj(graph_feats_all)
        # text_rep = self.new_text_proj(text_input)
        text_rep = text_input
        
        # root_vecs = torch.cat((root_vecs, text_rep), dim=1)
        root_vecs, kl_div = self.rsample(root_vecs, self.R_mean, self.R_var)
    ...
    def forward_encoder(self, graphs, tensors, graph_blip, orders, text_input):
        graph = Data(x=torch.from_numpy(np.stack(graph_blip[0])).cuda(), edge_index=torch.from_numpy(np.stack(graph_blip[1])).cuda(), edge_attr=torch.from_numpy(np.stack(graph_blip[2])).cuda())
        graph.batch = torch.from_numpy(np.stack(graph_blip[3])).cuda()
        text_input = torch.from_numpy(np.stack(text_input)).cuda()
        # batch_node, batch_mask = self.encoder(graph)
        ...
        # return root_vecs, root_vecs
        return root_vecs, text_rep
    
    def forward_text(self, graphs, tensors, graph_blip, orders, text_input):
        text_input = torch.from_numpy(np.stack(text_input)).cuda()
        return text_input

    def forward_decoder(self, root_vecs, context, greedy=True):
        # root_vecs = self.contrast(root_vecs, context.squeeze(), feature_only=True)
        return self.decoder.decode((root_vecs, root_vecs, root_vecs), greedy=greedy, max_decode_step=100)
    def train_decoder(self, graphs, tensors, graph_blip, orders, text_input, root_vecs, beta=0.1, perturb_z=True):
        tensors = make_cuda(tensors)
        loss, wacc, iacc, tacc, sacc = self.decoder((root_vecs, root_vecs, root_vecs), graphs, tensors, orders)
        return loss, wacc.item(), iacc.item(), tacc.item(), sacc.item()

    def forward(self, graphs, tensors, graph_blip, orders, text_input, beta=0.1, perturb_z=True):
        tree_tensors, graph_tensors = tensors = make_cuda(tensors)
        # print(graph_blip[0])
        ...
        # root_vecs = self.encoder.pool(root_vecs[batch_mask], graph.batch)
