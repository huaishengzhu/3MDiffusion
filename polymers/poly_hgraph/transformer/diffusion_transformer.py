...
class DiffusionTransformer(nn.Module):
    ...
    def forward(self, x, time, x_self_cond = None):
        ...
        if self.cross:
            ...
            x = self.encoder(tx_input, mask=self.mask, context=context, context_mask=self.context_mask, time_emb=time_emb)
        else:
            x = self.encoder(tx_input, mask=self.mask, time_emb=time_emb)
