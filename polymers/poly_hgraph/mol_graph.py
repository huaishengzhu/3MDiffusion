...
def tokenizer_text(text, text_tokenizer, text_max_len=128):
    sentence_token = text_tokenizer(text=text,
                                    truncation=True,
                                    padding='max_length',
...
class MolGraph(object):
    ...
    def pool_clusters(self):
        ...
        for fsmiles, fatoms in fragments:
            # print('C1=[CH:1][CH2:2]N=[CH:2][NH:2]1' in fragments)
    ...
    @staticmethod
    def tensorize(mol_batch, vocab, avocab, batch_text, model_blip):
        # for x in mol_batch:
        #     print(x)
        # print(vocab)
        smiles = mol_batch
        ...
        token_text = model_blip.blip2qformer.tokenizer
        ...
        for temp in batch_text:
            text, mask = tokenizer_text(temp, token_text) 
            texts.append(text.squeeze(0))
            masks.append(mask.squeeze(0))
        texts = torch.stack(texts).cuda()
        masks = torch.stack(masks).cuda()
        text_output = model_blip.blip2qformer.text_proj(model_blip.blip2qformer.Qformer.bert(texts, attention_mask=masks, return_dict=True).last_hidden_state[:, 0, :]).detach().cpu().numpy() 
        # start_time = time.time()
        # text_output = model_blip.blip2qformer.Qformer.bert(texts, attention_mask=masks, return_dict=True).last_hidden_state[:, 0, :].detach().cpu().numpy() 
        # end_time = time.time()
        # print("Running Time:", (end_time - start_time)/len(text_output))
        batch_graph = process_graph([smiles2graph(x) for x in smiles])
        tree_tensors = tree_tensors[:4] + (cgraph, tree_scope)
        return (tree_batchG, graph_batchG), (tree_tensors, graph_tensors), (batch_graph.x, batch_graph.edge_index, batch_graph.edge_attr, batch_graph.batch), all_orders, text_output
    
    @staticmethod
    def tensorize_text(batch_text, model_blip):
        token_text = model_blip.blip2qformer.tokenizer
        texts = []
        masks = []
        for temp in batch_text:
        ...
        texts = torch.stack(texts).cuda()
        masks = torch.stack(masks).cuda()
        text_output = model_blip.blip2qformer.text_proj(model_blip.blip2qformer.Qformer.bert(texts, attention_mask=masks, return_dict=True).last_hidden_state[:, 0, :]).detach()
        # text_output = model_blip.blip2qformer.Qformer.bert(texts, attention_mask=masks, return_dict=True).last_hidden_state[:, 0, :].detach()
        return text_output
