...
class HierMPNEncoder(nn.Module):
    ...
    def __init__(self, vocab, avocab, rnn_type, embed_size, hidden_size, depthT, depthG, dropout, decode=False):
    ...
    def forward(self, tree_tensors, graph_tensors):
        ...
        # print(graph_tensors[-1])
        # for st,le in graph_tensors[-1]:
        #     print(torch.mean(hatom[st: st+le], dim=0).shape)
        hroot = torch.stack([torch.mean(hatom[st: st+le], dim=0) for st,le in graph_tensors[-1]])
        # for ind in tree_tensors[-1]
...
class IncHierMPNEncoder(HierMPNEncoder):
    ...
    def __init__(self, vocab, avocab, rnn_type, embed_size, hidden_size, depthT, depthG, dropout):
        super(IncHierMPNEncoder, self).__init__(vocab, avocab, rnn_type, embed_size, hidden_size, depthT, depthG, dropout, decode=True)
